...
```{r}
library(monocle3)
celldataset <- new_cell_data_set(expression_data = exp(as.matrix(assay(study_mae, "rna"))), cell_metadata = coldata)
celldataset <- preprocess_cds(celldataset, method = "PCA", num_dim = 100, pseudo_count = 0, scaling = FALSE)
celldataset <- reduce_dimension(celldataset, max_components = 30, reduction_method = "UMAP", umap.metric = "cosine", cores = 4, umap.fast_sgd = TRUE)

celldataset <- cluster_cells(cds = celldataset, reduction_method = "UMAP")
celldataset <- learn_graph(cds = celldataset, use_partition = FALSE, close_loop = FALSE)
...
```{r}
umap_df <- data.frame(celldataset@reducedDims$UMAP)
ggplot(umap_df, aes(X1, X2)) + geom_point(aes(col = celldataset@colData$stage_lineage))
```
