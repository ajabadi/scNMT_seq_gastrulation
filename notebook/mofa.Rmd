...
`sparsity = TRUE`
`numFactors = 2`
`TrainOptions$DropFactorThreshold = 0` since we keep only 2 factors we won't require min variance explained.
...
```{r}
CLL_data$Mutations <- NULL
```

```{r, eval=params$on_mac, echo=params$on_mac}
## subset for faster initial runs
CLL_data <- lapply(CLL_data, subset_pn)
```
...
```{r MOFA-run-CLL, include=FALSE, echo=TRUE}
runtimes$CLL$MOFA <-  system.time({
...
```{r mixOmics-run-CLL}
X <- lapply(CLL_data[c("Drugs", "Methylation")], t)
Y <- t(CLL_data$mRNA)
keeps <- 10
ncomp = 2
keepX <- rep(list(rep(keeps, ncomp)), length(X)) %>% set_names(names(X))
keepY <- rep(keeps, ncomp)
```

```{r}
runtimes$CLL$spls <-  system.time({
  spls_res <- block.spls(X = X, Y = Y, scale = TRUE, keepX = keepX , keepY = keepY)
})
```
