...
for (data_name in epi_data) {
  ...
  bm_datasets_res[[data_name]] <- benchmark_nipals(dataset = dataset, weights = weights, props = props_NA, pheno = pheno, subset = subset, ncomp = ncomp)
}
```

```{r saveruns2, eval=params$rerun&params$saveRDS}
saveRDS(bm_datasets_res, file = here("output", "nipals-benchmark-results_all-datasets.rds"))
```

```{r}
# show_benchmark_res(bm_res = bm_datasets_res[[data_name]], which = 1, empca = TRUE)
```
