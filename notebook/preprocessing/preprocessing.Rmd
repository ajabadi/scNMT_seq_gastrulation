...
installer(pkgs = c("data.table", "magrittr", "scater", "purrr"))
```
...
```{r}
dcast_met_wt <- dcast_dt_list(met_dt_list, row = "id", col = "id_met", value.var = "wij")
```


```{r}
lapply(dcast_met, dim)
lapply(dcast_met_wt, dim)
```
...
```{r}
dcast_acc_wt <- dcast_dt_list(acc_dt_list, row = "id", col = "id_acc", value.var = "wij")
```
...
```{r}
sce_matching <- readRDS("/Users/alabadi/Projects/analysis/R/multiOmics/scNMT_seq_gatrulation/__Hackathon/easy-data/data/rna/parsed/SingleCellExperiment_matching-cells.rds")
```
...
exper.list <- ExperimentList(rna = logcounts(sce_matching),
                             ...
                             ## weights
                             wt_met_genebody = dcast_met_wt$Genebody,
                             wt_met_promoter = dcast_met_wt$Promoters,
                             wt_met_cgi = dcast_met_wt$CGI,
                             wt_met_p300 = dcast_met_wt$p300,
                             wt_met_CTCF = dcast_met_wt$CTCF,
                             wt_met_DHS = dcast_met_wt$DHS,

                             wt_acc_genebody = dcast_acc_wt$Genebody,
                             wt_acc_promoter = dcast_acc_wt$Promoters,
                             wt_acc_cgi = dcast_acc_wt$CGI,
                             wt_acc_p300 = dcast_acc_wt$p300,
                             wt_acc_CTCF = dcast_acc_wt$CTCF,
                             wt_acc_DHS = dcast_acc_wt$DHS
)
...
```{r}
scnmtseq_gastrulation_mae <- readRDS(file.path(io$outdir, "scnmtseq_gastrulation_mae_826-cells_orderedFeatures.rds"))
```
